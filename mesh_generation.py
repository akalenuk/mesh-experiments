...
def triangulate(sdf, p0, scale, subdivisions, ts, vs):
    if subdivisions > 0:
        ...
        p12 = sum_of(p0, scaled(a1, scale / 2.) + scaled(a2, scale / 2.))
        p23 = sum_of(p0, scaled(a2, scale / 2.) + scaled(a3, scale / 2.))
        p31 = sum_of(p0, scaled(a3, scale / 2.) + scaled(a1, scale / 2.))
        p123 = sum_of(p0, scaled(a1, scale / 2.) + scaled(a2, scale / 2.) + scaled(a3, scale / 2.))
        ...
        p12 = sum_of(p0, scaled(a1, scale) + scaled(a2, scale))
        p23 = sum_of(p0, scaled(a2, scale) + scaled(a3, scale))
        p31 = sum_of(p0, scaled(a3, scale) + scaled(a1, scale))
        p123 = sum_of(p0, scaled(a1, scale) + scaled(a2, scale) + scaled(a3, scale))
        ps = [p0, p1, p2, p3, p12, p23, p31, p123]
